...
const HomePage = ({ lang, siteSettings, menuItems, handleOpenItemDetails, cart, setCart }) => {
  ...
  return (
    <div className="bg-[#12080A] min-h-screen text-white">
      <header className="relative w-full min-h-[520px] md:min-h-[620px] bg-[#12080A] flex flex-col items-center justify-center overflow-hidden border-b-4 border-[#FF4500] shadow-2xl py-12">
        <div 
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto flex flex-col items-center">
          <h1 className="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-[#FF4500] via-[#FF6347] to-[#E63900] drop-shadow-[0_5px_5px_rgba(0,0,0,0.9)] mb-4">
            {title}
          </h1>
          <Link to="/menu" className="inline-block bg-[#FF4500] hover:bg-[#E63900] text-white border-2 border-[#FF4500] px-8 py-3 text-xl font-black rounded-2xl hover:scale-105 transition shadow-[0_0_30px_rgba(255,69,0,0.4)] mb-6">
            {t.orderNow}
          </Link>
          ...
          {siteSettings.promoBannerImage && (
            <div className="w-full max-w-xl mt-3 px-4">
              <img 
                ...
                alt="Banner Offer" 
                className="w-full h-auto max-h-[320px] object-cover rounded-2xl border-2 border-[#FF4500] shadow-[0_0_30px_rgba(255,69,0,0.4)]" 
              />
            </div>
          )}
        </div>
      ...
      <section className="px-8 py-14 max-w-7xl mx-auto relative">
        <div className="flex justify-between items-center mb-8 border-b border-[#3A1218] pb-4">
          <h2 className="text-3xl font-bold text-[#FF4500]">{t.bestOffers}</h2>
        ...
        {offerItems.length > 0 ? (
          <div className="relative overflow-hidden px-4">
            {offerItems.length > 3 && (
              <>
                <button onClick={prevSlide} className="absolute left-0 top-1/2 -translate-y-1/2 z-20 bg-[#FF4500] text-white w-10 h-10 rounded-full font-black text-xl flex items-center justify-center shadow-lg hover:bg-[#E63900] transition">❮</button>
                <button onClick={nextSlide} className="absolute right-0 top-1/2 -translate-y-1/2 z-20 bg-[#FF4500] text-white w-10 h-10 rounded-full font-black text-xl flex items-center justify-center shadow-lg hover:bg-[#E63900] transition">❯</button>
              </>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 transition-all duration-300">
              {offerItems.slice(currentIndex, currentIndex + 3).map((item) => {
                ...
                return (
                  <div 
                    key={item._id} 
                    className="bg-[#1C0D10] border border-[#3A1218] rounded-2xl overflow-hidden shadow-2xl hover:border-[#FF4500] transition duration-300 group flex flex-col relative pt-8"
                  >
                    {item.discount > 0 && (
                      <div className="absolute top-0 inset-x-0 z-20 bg-gradient-to-r from-[#FF4500] via-[#FF7300] to-[#FF4500] text-white text-center py-2 font-black text-base md:text-lg shadow-xl border-b-2 border-black/20 tracking-wider">
                        🔥 خصم  {item.discount}%  🔥
                      </div>
                    )}
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {item.discount > 0 ? (
                            <>
                              ...
                              <span className="text-[#FF4500] font-black text-lg">{finalPrice} ج</span>
                            </>
                          ) : (
                            <span className="text-[#FF4500] font-black text-lg">{item.price} ج</span>
                          )}
                        </div>
                      </div>
                      
                      {quantity === 0 ? (
                        <button onClick={() => handleOpenItemDetails(item)} className="bg-[#FF4500] text-white px-4 py-2 rounded-xl font-bold text-sm shadow hover:bg-[#E63900] transition">
                          أضف للسلة 🛒
                        </button>
                      ) : (
                        <div className="flex items-center bg-[#12080A] border border-[#FF4500] rounded-xl px-2 py-1 gap-2">
                          <button onClick={() => {
...
const MenuPage = ({ menuItems, categories, lang, handleOpenBox, handleOpenItemDetails, cart, setCart }) => {
  ...
  return (
    <section className="px-8 py-12 max-w-7xl mx-auto min-h-screen relative bg-[#12080A] text-white">
      <h2 className="text-3xl font-bold text-[#FF4500] mb-6 border-b border-[#3A1218] pb-4">{t.ourMenu}</h2>
        ...
        <button
          ...
          className={`px-6 py-2.5 rounded-full font-bold whitespace-nowrap transition border ${
            selectedCategory === 'الكل' || selectedCategory === 'All'
              ? 'bg-[#FF4500] text-white border-[#FF4500] shadow-[0_0_15px_rgba(255,69,0,0.5)]' 
              : 'bg-[#1C0D10] text-zinc-300 border-[#3A1218] hover:border-[#FF4500]'
          }`}
        >
          {t.all}
        </button>
          ...
          {categoriesToShow.map(cat => {
            ...
            if (catItems.length === 0 && selectedCategory !== 'الكل' && selectedCategory !== 'All') {
              return (
                <div key={cat._id} className="border-b border-[#3A1218] pb-10">
                  <h3 className="text-2xl font-black text-[#FF4500] mb-6 border-r-4 border-[#FF4500] pr-3">{cat.name}</h3>
            ...
            return (
              <div key={cat._id} className="border-b border-[#3A1218] pb-12">
                <h3 className="text-2xl font-black text-[#FF4500] mb-6 border-r-4 border-[#FF4500] pr-3">
                  {cat.name}
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                  {catItems.map(item => {
                    ...
                    return (
                      <div 
                        key={item._id} 
                        className="bg-[#1C0D10] border border-[#3A1218] rounded-2xl overflow-hidden flex flex-col hover:border-[#FF4500] transition group shadow-xl relative pt-8"
                      >
                        {item.discount > 0 && (
                          <div className="absolute top-0 inset-x-0 z-20 bg-gradient-to-r from-[#FF4500] via-[#FF7300] to-[#FF4500] text-white text-center py-2 font-black text-base md:text-lg shadow-xl border-b-2 border-black/20 tracking-wider">
                            🔥 خصم  {item.discount}%   🔥
                        ...
                        <div onClick={() => item.type === 'box' ? handleOpenBox(item) : handleOpenItemDetails(item)} className="w-full h-48 object-cover bg-[#12080A] overflow-hidden cursor-pointer">
                          <img src={item.image || "https://via.placeholder.com/400x300/222/FF4500?text=Bahbah+Burger"} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                        </div>
                          ...
                          <div className="flex items-center gap-3 mb-4 mt-auto">
                            {item.discount > 0 ? (
                              <>
                                ...
                                <span className="text-[#FF4500] text-2xl font-black">{finalPrice} ج</span>
                              </>
                            ) : (
                              <span className="text-[#FF4500] text-2xl font-black">{item.price} ج</span>
                            )}
                          </div>
                          
                          {item.type === 'box' ? (
                              <button onClick={() => handleOpenBox(item)} className="w-full bg-[#FF4500] text-white font-bold py-2.5 rounded-xl hover:bg-[#E63900] transition shadow">
                              {t.customizeBox}
                              </button>
                          ) : (
                              quantity === 0 ? (
                                <button onClick={() => handleOpenItemDetails(item)} className="w-full border-2 border-[#FF4500] text-[#FF4500] font-bold py-2.5 rounded-xl hover:bg-[#FF4500] hover:text-white transition shadow">
                                  {t.details} 🛒
                              </button>
                            ) : (
                              <div className="flex items-center justify-between bg-[#12080A] border border-[#FF4500] rounded-xl px-3 py-2">
                                <span className="text-xs text-[#FF4500] font-bold">الكمية:</span>
                                <div className="flex items-center gap-3">
                                  <button onClick={() => {
                                    const idx = cart.findIndex(i => i.name === item.name);
                                    if (idx !== -1) { const nc = [...cart]; nc.splice(idx, 1); setCart(nc); }
                                  }} className="w-7 h-7 bg-[#1C0D10] rounded-lg text-[#FF4500] font-black hover:bg-[#FF4500] hover:text-white">-</button>
                                  <span className="font-black text-white">{quantity}</span>
                                  <button onClick={() => setCart([...cart, { ...item, price: finalPrice }])} className="w-7 h-7 bg-[#1C0D10] rounded-lg text-[#FF4500] font-black hover:bg-[#FF4500] hover:text-white">+</button>
                                </div>
                              </div>
                        )
                      )}
...
const AdminDashboard = ({ menuItems, categories, siteSettings, lang, fetchItems, fetchCategories, fetchSettings, isAuthenticated }) => {
  ...
  const handleSaveItem = async (e) => {
    ...
    const itemData = {
      ...
      image: image || "https://via.placeholder.com/400x300/222/FF4500?text=Bahbah+Burger",
      description,
  ...
  return (
    <section className="px-8 py-12 max-w-5xl mx-auto min-h-[80vh] bg-[#12080A] text-white">
      <div className="flex justify-between items-center mb-8 border-b border-[#3A1218] pb-4">
        <h2 className="text-3xl font-bold text-[#FF4500]">⚙️ لوحة الإدارة</h2>
        <Link className="text-zinc-400 hover:text-white underline" to="/menu">{t.menu}</Link>
      </div>

      <div className="bg-[#1C0D10] p-6 rounded-2xl border border-[#FF4500]/60 mb-8 shadow-xl">
        <h3 className="text-xl font-bold text-[#FF4500] mb-4">🚚 إدارة مناطق التوصيل وأسعارها</h3>
        <form onSubmit={handleAddZone} className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          />
          <button type="submit" className="bg-[#FF4500] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#E63900] transition shadow cursor-pointer">
            ➕ إضافة منطقة
          </button>
        </form>
          ...
          {deliveryZones.length === 0 ? (
            ...
            deliveryZones.map((zone) => (
              <div key={zone._id} className="bg-[#12080A] border border-[#3A1218] px-4 py-3 rounded-xl flex items-center justify-between text-sm">
                <span className="font-bold text-[#FF4500]">{zone.name} — <span className="text-white">{zone.fee} جنيه</span></span>
      ...
      <form onSubmit={handleSaveSettings} className="bg-[#1C0D10] p-6 rounded-2xl border border-[#FF4500]/50 mb-8 shadow-xl">
        <h3 className="text-xl font-bold text-[#FF4500] mb-4">{t.siteSettings}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
          ...
          <div className="md:col-span-2 bg-[#12080A] p-4 rounded-xl border border-[#FF4500]/30">
            <label className="block text-sm mb-2 text-[#FF4500] font-bold">🖼️ صورة العرض الكبيرة (البوستر تحت زرار اطلب دلوقتي)</label>
        ...
        <button type="submit" className="w-full bg-[#FF4500] text-white font-bold py-3 rounded-xl hover:bg-[#E63900] transition shadow">
          💾 حفظ تعديلات اللوجو والواجهة وصورة العرض
        </button>
      </form>

      <div className="bg-[#1C0D10] p-6 rounded-2xl border border-[#3A1218] mb-8 shadow-xl">
        <h3 className="text-xl font-bold text-[#FF4500] mb-4">{t.catManage}</h3>
        <form onSubmit={handleSaveCategory} className="flex gap-4 mb-6">
          ...
          />
          <button type="submit" className="bg-[#FF4500] text-white font-bold px-6 py-3 rounded-xl hover:bg-[#E63900] transition shadow">
          ...
          {categories.map((cat, index) => (
            <div key={cat._id} className="bg-[#12080A] border border-[#3A1218] px-4 py-3 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-[#FF4500]">{cat.name}</span>
              </div>
              <div className="flex items-center gap-2">
                <button 
                  ...
                  disabled={index === 0}
                  className={`px-3 py-1.5 rounded-lg text-sm font-bold ${index === 0 ? 'bg-[#1C0D10] text-zinc-600 cursor-not-allowed' : 'bg-[#220E13] text-[#FF4500] hover:bg-[#2A080D]'}`}
                >
                  ◀ تحريك للخارج
                </button>
                <button 
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-bold ${index === categories.length - 1 ? 'bg-[#1C0D10] text-zinc-600 cursor-not-allowed' : 'bg-[#220E13] text-[#FF4500] hover:bg-[#2A080D]'}`}
                >
                  تحريك للداخل ▶
                </button>
                <button onClick={() => handleEditCategoryClick(cat)} className="text-[#FF4500] bg-[#FF4500]/20 px-3 py-1.5 rounded-lg text-xs font-bold">✏️</button>
    ...
    <form onSubmit={handleSaveItem} className="bg-[#1C0D10] p-6 rounded-2xl border border-[#3A1218] mb-8 grid grid-cols-1 md:grid-cols-2 gap-4 shadow-xl">
      <h3 className="md:col-span-2 text-xl font-bold text-[#FF4500] mb-2">{t.itemManage}</h3>
      <div>
      ...
      <div>
        <label className="block text-sm mb-2 text-[#FF4500]">نسبة الخصم % (اختياري)</label>
        <input type="number" placeholder="مثال: 20" value={discount} onChange={(e) => setDiscount(e.target.value)} className="w-full bg-[#12080A] border border-[#FF4500] rounded-xl p-3 text-white" />
      </div>
      <div>
      ...
      <div className="md:col-span-2 bg-[#12080A] p-4 rounded-xl border border-[#FF4500]/30 flex items-center gap-3">
        <input 
          type="checkbox" 
          id="isOfferCheck" 
          checked={isOffer} 
        ...
        />
        <label htmlFor="isOfferCheck" className="text-[#FF4500] font-bold cursor-pointer">
      ...
      {type === 'box' && (
        <div>
          <label className="block text-sm mb-2 text-[#FF4500]">Max Items in Box *</label>
          <input type="number" value={maxItems} onChange={(e) => setMaxItems(e.target.value)} className="w-full bg-[#12080A] border border-[#FF4500] rounded-xl p-3 text-white" placeholder="3" />
        </div>
      )}

      {type === 'box' && (
        <div className="md:col-span-2 bg-[#12080A] p-4 rounded-xl border border-[#FF4500]/50">
          <label className="block text-sm mb-2 text-[#FF4500] font-bold">📦 أسماء المكونات التي تظهر داخل البوكس</label>
          <div className="flex gap-2 mb-3">
            ...
            />
            <button type="button" onClick={handleAddBoxItemName} className="bg-[#FF4500] text-white px-5 rounded-xl font-bold text-sm hover:bg-[#E63900]">
              ➕ إضافة
            </button>
          </div>
            ...
            <div className="space-y-2 mt-2">
              {boxItemsList.map((bItem, index) => (
                <div key={index} className="flex justify-between items-center bg-[#12080A] px-3 py-2 rounded-xl border border-[#3A1218] text-sm">
                  <span className="text-[#FF4500] font-bold">{bItem.name}</span>
      ...
      <div className="md:col-span-2 bg-[#12080A] p-4 rounded-xl border border-[#FF4500]/50">
          <label className="block text-sm mb-2 text-[#FF4500] font-bold">⚖️ أحجام الصنف وأسعارها (مثل: كيلو، نص، ربع)</label>
          <div className="flex gap-2 mb-3">
            <input 
              type="text" 
            ...
            />
            <button type="button" onClick={handleAddSize} className="bg-[#FF4500] text-white px-4 rounded-xl font-bold text-sm hover:bg-[#E63900]">
              ➕ إضافة حجم
            </button>
          </div>
          ...
          {sizesList.length > 0 && (
            <div className="space-y-2 mt-2">
              {sizesList.map((sz, index) => (
                <div key={index} className="flex justify-between items-center bg-[#12080A] px-3 py-2 rounded-xl border border-[#3A1218] text-sm">
                  <span className="text-[#FF4500] font-bold">{sz.name} — {sz.price} ج</span>
      ...
      <div className="md:col-span-2 bg-[#12080A] p-4 rounded-xl border border-[#3A1218]">
          <label className="block text-sm mb-2 text-[#FF4500] font-bold">✨ الإضافات الاختيارية</label>
          <div className="flex gap-2 mb-3">
            <input 
              type="text" 
            ...
            <input 
              ...
              className="w-32 bg-[#1C0D10] border border-[#3A1218] rounded-xl p-2.5 text-white text-sm"
            />
            <button type="button" onClick={handleAddAddon} className="bg-[#FF4500] text-white px-4 rounded-xl font-bold text-sm hover:bg-[#E63900]">
              ➕ إضافة
            </button>
          </div>
      ...
      <div className="md:col-span-2">
          <label className="block text-sm mb-2 text-[#FF4500] font-bold">Description *</label>
      ...
      <div className="md:col-span-2 mt-4 flex gap-4">
          <button type="submit" className="flex-1 bg-[#FF4500] text-white font-bold py-3.5 rounded-xl hover:bg-[#E63900] transition shadow">
    ...
    <div className="space-y-10 mt-10">
      <h3 className="text-2xl font-bold text-[#FF4500] border-b border-[#3A1218] pb-3">📋 إدارة وترتيب الأصناف حسب الأقسام</h3>
      ...
      {categories.map(cat => {
        ...
        return (
          <div key={cat._id} className="bg-[#1C0D10] border border-[#3A1218] rounded-2xl p-6 shadow-xl">
            <h4 className="text-xl font-black text-[#FF4500] mb-4 border-r-4 border-[#FF4500] pr-3">
                  ...
                  <div key={item._id} className="bg-[#12080A] border border-[#3A1218] p-3.5 rounded-xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold ${itemIndex === 0 ? 'bg-[#1C0D10] text-zinc-600 cursor-not-allowed' : 'bg-[#220E13] text-[#FF4500] hover:bg-[#2A080D]'}`}
                      >
                        ▲
                      </button>
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold ${itemIndex === catItems.length - 1 ? 'bg-[#1C0D10] text-zinc-600 cursor-not-allowed' : 'bg-[#220E13] text-[#FF4500] hover:bg-[#2A080D]'}`}
                      >
                        ▼
                      </button>
                      <button onClick={() => handleEditItemClick(item)} className="text-[#FF4500] bg-[#FF4500]/30 px-3.5 py-1.5 rounded-lg text-xs font-bold">✏️ تعديل</button>
...
const CartPage = ({ cart, setCart, lang }) => {
    ...
    return (
      <section className="px-8 py-12 max-w-4xl mx-auto min-h-[60vh] bg-[#12080A] text-white flex flex-col items-center justify-center">
        <div className="bg-[#1C0D10] border border-[#25D366] rounded-2xl p-10 text-center shadow-[0_0_20px_rgba(37,211,102,0.2)] w-full">
          ...
          <div className="bg-[#12080A] border-2 border-[#FF4500] text-[#FF4500] text-4xl font-black py-4 px-8 rounded-xl inline-block mb-8 tracking-widest">
            {placedOrderId}
          </div>
          <p className="text-sm text-zinc-400 mb-8">تم تحويلك للواتساب لإرسال الطلب للمطعم.</p>
          <button 
            onClick={() => setPlacedOrderId(null)} 
            className="text-white bg-[#FF4500] hover:bg-[#E63900] px-8 py-3 rounded-xl font-bold transition shadow-lg"
          >
            رجوع للسلة
          </button>
        </div>
  ...
  return (
    <section className="px-8 py-12 max-w-4xl mx-auto min-h-screen bg-[#12080A] text-white">
      <h2 className="text-3xl font-bold text-[#FF4500] mb-8 border-b border-[#3A1218] pb-4">{t.cart}</h2>
      ...
      {cart.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-[#1C0D10] rounded-2xl p-6 border border-[#3A1218] shadow-xl">
            <h3 className="text-xl font-bold text-[#FF4500] mb-4">محتويات السلة</h3>
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
              {groupedCart.map((item, index) => (
                <div key={index} className="flex justify-between items-center border-b border-[#3A1218] pb-3">
                  <div>
                    <h4 className="text-base font-bold text-white">{item.name}</h4>
                    <p className="text-[#FF4500] font-bold text-sm">{item.price * item.quantity} ج</p>
                  </div>
            ...
            <div className="mt-6 pt-4 border-t border-[#3A1218] space-y-2 text-sm text-zinc-300">
              ...
              {orderType === 'delivery' && (
                <div className="flex justify-between"><span>سعر التوصيل:</span><span className="font-bold text-[#FF4500]">{deliveryFee} ج</span></div>
              )}
          </div>

          <div className="mt-4 pt-4 border-t-2 border-[#FF4500] flex justify-between items-center">
            <span className="text-lg font-bold">{t.total}</span>
            <span className="text-2xl font-black text-[#FF4500]">{grandTotal} جنيه</span>
          </div>
        </div>

        <div className="bg-[#1C0D10] rounded-2xl p-6 border border-[#3A1218] flex flex-col justify-between shadow-xl">
          <div className="space-y-4">
            <h3 className="text-xl font-bold text-[#FF4500] mb-2">بيانات التوصيل والاستلام</h3>
            
            <div className="grid grid-cols-2 gap-3 mb-2">
              <button 
                type="button"
                onClick={() => setOrderType('delivery')}
                className={`py-3 rounded-xl font-bold text-sm transition ${orderType === 'delivery' ? 'bg-[#FF4500] text-white border border-[#FF4500]' : 'bg-[#12080A] text-zinc-400 border border-[#3A1218]'}`}
              >
                🛵 توصيل دليفري
              </button>
              <button 
                ...
                className={`py-3 rounded-xl font-bold text-sm transition ${orderType === 'pickup' ? 'bg-[#FF4500] text-white border border-[#FF4500]' : 'bg-[#12080A] text-zinc-400 border border-[#3A1218]'}`}
              >
                🏪 استلام من الفرع
              </button>
          </div>
...
function App() {
  ...
  return (
    <div dir={lang === 'ar' ? 'rtl' : 'ltr'} className="min-h-screen bg-[#12080A] text-white font-sans flex flex-col justify-between relative">
      <nav className="bg-[#1C0D10] border-b border-[#FF4500]/60 sticky top-0 z-50 shadow-2xl">
        <div className="flex items-center justify-between px-8 py-3">
          ...
          <ul className="hidden md:flex gap-4 text-base font-bold">
            <li><Link to="/" className="bg-[#12080A] hover:bg-[#FF4500] text-zinc-300 hover:text-white border border-[#3A1218] px-6 py-2 rounded-xl transition shadow">{t.home}</Link></li>
            <li><Link to="/menu" className="bg-[#12080A] hover:bg-[#FF4500] text-zinc-300 hover:text-white border border-[#3A1218] px-6 py-2 rounded-xl transition shadow">{t.menu}</Link></li>
          </ul>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="bg-[#12080A] text-zinc-300 border border-[#3A1218] px-3 py-2 rounded-xl text-sm font-bold hover:text-white hover:border-[#FF4500] transition"
            >
              {lang === 'ar' ? 'EN' : 'عربي'}
            </button>

            <Link to="/cart" className="flex items-center gap-2 bg-[#FF4500] text-white px-5 py-2.5 rounded-xl font-bold hover:bg-[#E63900] transition shadow-lg">
              <span>🛒 {t.cart}</span>
              <span className="bg-[#12080A] text-[#FF4500] px-2 py-0.5 rounded-full text-xs font-black">{cart.length}</span>
            </Link>
          </div>
        </div>
      </nav>
      ...
      {selectedItemDetail && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C0D10] border border-[#FF4500] rounded-2xl w-full max-w-lg p-6 relative shadow-2xl">
            ...
            {selectedItemDetail.sizes && selectedItemDetail.sizes.length > 0 && (
              <div className="mb-6 space-y-3">
                <h4 className="text-sm font-bold text-[#FF4500]">اختر الحجم:</h4>
                <div className="grid grid-cols-2 gap-3">
                  {selectedItemDetail.sizes.map((sz, idx) => {
                    const finalSzPrice = getDiscountedPrice(sz.price, selectedItemDetail.discount);
                    return (
                      <div key={idx} onClick={() => setSelectedSize(sz)} className={`p-3 rounded-xl border cursor-pointer flex flex-col items-center justify-center transition ${selectedSize === sz ? 'bg-[#FF4500]/30 border-[#FF4500] text-[#FF4500]' : 'bg-[#12080A] border-[#3A1218] text-zinc-300'}`}>
                        ...
                        {selectedItemDetail.discount > 0 ? (
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-sm font-black text-[#FF4500]">{finalSzPrice} ج</span>
                          </div>
                        ) : (
                          <span className="text-sm font-black text-[#FF4500]">{sz.price} ج</span>
                        )}
                      </div>
                    );
                  })}
            ...
            {selectedItemDetail.addons && selectedItemDetail.addons.length > 0 && (
              <div className="mb-6 space-y-3">
                <h4 className="text-sm font-bold text-[#FF4500]">✨ الإضافات الاختيارية:</h4>
                <div className="flex flex-col gap-3">
                    
                  <label className={`p-3 rounded-xl border cursor-pointer flex items-center justify-between transition ${!selectedAddon ? 'bg-[#FF4500]/30 border-[#FF4500] text-[#FF4500]' : 'bg-[#12080A] border-[#3A1218] text-zinc-300'}`}>
                    <div className="flex items-center gap-3">
                      <input type="radio" name="addon" checked={!selectedAddon} onChange={() => setSelectedAddon(null)} className="hidden" />
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${!selectedAddon ? 'border-[#FF4500]' : 'border-[#3A1218]'}`}>
                  ...
                  {selectedItemDetail.addons.map((addon, idx) => (
                    <label key={idx} className={`p-3 rounded-xl border cursor-pointer flex items-center justify-between transition ${selectedAddon === addon ? 'bg-[#FF4500]/30 border-[#FF4500] text-[#FF4500]' : 'bg-[#12080A] border-[#3A1218] text-zinc-300'}`}>
                      <div className="flex items-center gap-3">
                        <input type="radio" name="addon" checked={selectedAddon === addon} onChange={() => setSelectedAddon(addon)} className="hidden" />
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${selectedAddon === addon ? 'border-[#FF4500]' : 'border-[#3A1218]'}`}>
                          {selectedAddon === addon && <div className="w-2.5 h-2.5 bg-[#FF4500] rounded-full"></div>}
                        </div>
                        <span className="font-bold text-sm">{addon.name}</span>
                      </div>
                      <span className="text-sm font-black text-[#FF4500]">+{addon.price} ج</span>
                    </label>
                  ))}
            ...
            <button onClick={handleAddCustomizedItemToCart} className="w-full bg-[#FF4500] text-white font-black py-4 rounded-xl hover:bg-[#E63900] transition text-lg shadow-lg">
      ...
      {isBoxModalOpen && activeBox && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1C0D10] border border-[#FF4500] rounded-2xl w-full max-w-lg p-6 relative shadow-2xl">
            ...
            <h3 className="text-3xl font-black text-[#FF4500] mb-1">{activeBox.name}</h3>
            <p className="text-zinc-300 mb-6 border-b border-[#3A1218] pb-4 text-sm">
              اختر {activeBox.maxItems} أصناف. 
              <span className={`block mt-1 font-bold text-base ${totalSelected === activeBox.maxItems ? 'text-green-400' : 'text-[#FF4500]'}`}>
            ...
            <div className="space-y-4 mb-8 max-h-[50vh] overflow-y-auto pr-1">
              {activeBox.boxItems && activeBox.boxItems.map((bItem, idx) => (
                <div key={idx} className="flex justify-between items-center bg-[#12080A] p-3.5 rounded-xl border border-[#3A1218]">
                  ...
                  <div className="flex items-center gap-4">
                    <button onClick={() => handleUpdateSelection(bItem.name, 'remove')} className="w-8 h-8 bg-[#1C0D10] rounded-lg text-[#FF4500] font-bold">-</button>
                    <span className="text-xl w-4 text-center font-black text-white">{boxSelections[bItem.name] || 0}</span>
                    <button onClick={() => handleUpdateSelection(bItem.name, 'add')} className="w-8 h-8 bg-[#1C0D10] rounded-lg text-[#FF4500] font-bold">+</button>
                  </div>
                </div>
              ))}
            </div>

            <button onClick={handleAddBoxToCart} disabled={totalSelected !== activeBox.maxItems} className={`w-full py-4 rounded-xl font-black text-lg transition ${totalSelected === activeBox.maxItems ? 'bg-[#FF4500] text-white hover:bg-[#E63900] cursor-pointer shadow-lg' : 'bg-zinc-800 text-zinc-500 cursor-not-allowed'}`}>
              Add to Cart
            </button>
          </div>
        </div>
