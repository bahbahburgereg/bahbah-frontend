...
const Navbar = ({ lang, setLang, cartLength, siteSettings, onSecretClick }) => (
  // تم إضافة dir="ltr" لتثبيت اللوجو يساراً والسلة يميناً كما في التصميم
  <nav className="fixed top-0 w-full z-50 bg-[#050000]/95 border-b border-zinc-900 transition-all font-cairo" dir="ltr">
...
const HomePage = ({ lang, siteSettings, menuItems, categories, handleOpenItemDetails }) => {
  ...
  return (
    // تم إضافة dir="ltr" لتثبيت التكوين السينمائي (النصوص يساراً والصورة يميناً)
    <div className="bg-[#050000] min-h-screen text-white font-cairo overflow-hidden selection:bg-[#e11d48] selection:text-white" dir="ltr">
      ...
      <header className="relative w-full min-h-screen flex items-center justify-between px-6 md:px-16 pt-20">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between">
          ...
          {/* Text Side (Left) */}
          <div className="flex flex-col z-20 md:w-1/2 pt-10 items-start">
            ...
            <div className="transform -rotate-6 mt-4 flex flex-col w-full">
              <h1 className="font-anton text-[4.5rem] md:text-[6.5rem] lg:text-[8rem] leading-[0.85] grunge-text-white tracking-tight uppercase">
                THE FIRE
              </h1>
              <h1 className="font-anton text-[4.5rem] md:text-[6.5rem] lg:text-[8rem] leading-[0.85] grunge-text-red tracking-tight uppercase md:ml-16">
                IS COMING
              </h1>
            </div>

            {/* الحاوية دي عشان النص العربي والزرار يبقوا تحت كلمة IS COMING زي الصورة */}
            <div className="mt-8 flex flex-col items-end w-full max-w-[400px] md:ml-16">
              <p className="text-white text-lg font-bold tracking-wide w-full text-right" dir="rtl">
                مش مجرد برجر .. ده بحبح!
              </p>
              
              <button className="mt-6 bg-[#e11d48] hover:bg-red-700 text-white px-8 py-3.5 rounded font-bold shadow-lg transition flex items-center justify-center gap-2 text-sm w-fit" dir="rtl">
                {t.orderNow}
              </button>
            </div>
          </div>
      ...
      <section className="px-6 md:px-12 py-24 max-w-[1400px] mx-auto relative z-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-3 flex flex-col pt-8">
            ...
            <h3 className="text-lg font-black text-white" dir={lang === 'ar' ? 'rtl' : 'ltr'}>{t.menuSubtitle}</h3>
            <p className="text-zinc-500 text-xs mt-3 leading-loose max-w-[200px] font-bold" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
              {t.menuDesc}
            </p>
            <button className="mt-8 border border-zinc-800 text-white px-5 py-2.5 rounded text-xs font-bold w-fit hover:bg-[#e11d48] hover:border-[#e11d48] transition" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
              {t.viewAll}
            </button>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col">
            {/* Categories Pills */}
            <div className="flex gap-2 overflow-x-auto scrollbar-none mb-10 pb-2" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            {/* Product Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
                  ...
                  return (
                    <div key={item._id} className="card-bg border border-zinc-900 rounded-2xl overflow-hidden flex flex-col group relative hover:border-red-900/50 transition duration-300">
                      ...
                      <div className="px-5 pb-5 pt-2 flex flex-col flex-1">
                        <h4 className="font-anton text-lg text-white uppercase tracking-wide" dir="ltr">{item.name}</h4>
                        <p className="text-[10px] text-zinc-500 mt-1.5 line-clamp-2 leading-relaxed font-bold">{item.description}</p>
                        
                        <div className="flex items-center justify-between mt-auto pt-4" dir="ltr">
...
const CartPage = ({ cart, setCart, lang }) => {
  ...
  return (
    <section className="pt-32 pb-20 px-6 max-w-5xl mx-auto min-h-screen font-cairo text-white bg-[#050000]">
      <h2 className="text-4xl font-anton text-[#e11d48] tracking-widest uppercase mb-8" dir="ltr">{t.cart}</h2>
      {cart.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-[#110808] rounded-2xl p-6 border border-zinc-900">
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
              {groupedCart.map((item, index) => (
                <div key={index} className="flex justify-between items-center border-b border-zinc-900 pb-4">
                  <div>
                    <h4 className="text-sm font-bold text-white" dir="ltr">{item.name}</h4>
                    <p className="text-[#e11d48] font-anton tracking-wider" dir="ltr">{item.price * item.quantity} EGP</p>
                  </div>
            ...
            <div className="mt-6 pt-4 border-t border-zinc-900 flex justify-between items-center">
              ...
              <span className="text-3xl font-anton text-[#e11d48]" dir="ltr">{grandTotal} EGP</span>
            </div>
          </div>
          ...
          <div className="bg-[#110808] rounded-2xl p-6 border border-zinc-900 flex flex-col">
            ...
            <div className="space-y-4">
              ...
              <input type="text" maxLength="11" placeholder="رقم الموبايل (11 رقم)" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value.replace(/\D/g, ''))} className="w-full bg-black border border-zinc-900 rounded p-3 text-sm focus:border-[#e11d48] outline-none" dir="ltr" />
              {orderType === 'delivery' && (
                <>
...
const AdminDashboard = ({ menuItems, categories, siteSettings, lang, fetchItems, fetchCategories, fetchSettings, isAuthenticated }) => {
  ...
  return (
    <section className="pt-32 pb-20 px-6 max-w-5xl mx-auto min-h-screen font-cairo text-white bg-[#050000]">
      <h2 className="text-3xl font-anton text-[#e11d48] tracking-widest mb-8" dir="ltr">ADMIN PANEL</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-[#110808] p-6 rounded-2xl border border-zinc-900">
          ...
          <form onSubmit={handleAddZone} className="flex gap-2 mb-4">
            <input type="text" placeholder="المنطقة" value={zoneName} onChange={e => setZoneName(e.target.value)} className="w-1/2 bg-black border border-zinc-800 rounded p-2 text-sm outline-none focus:border-[#e11d48]" />
            <input type="number" placeholder="السعر" value={zoneFee} onChange={e => setZoneFee(e.target.value)} className="w-1/4 bg-black border border-zinc-800 rounded p-2 text-sm outline-none focus:border-[#e11d48]" />
        ...
        <div className="bg-[#110808] p-6 rounded-2xl border border-zinc-900">
          ...
          <form onSubmit={handleSaveCategory} className="flex gap-2 mb-4">
            <input type="text" placeholder="اسم القسم" value={catName} onChange={e => setCatName(e.target.value)} className="flex-1 bg-black border border-zinc-800 rounded p-2 text-sm outline-none focus:border-[#e11d48]" />
      ...
      <form onSubmit={handleSaveItem} className="bg-[#110808] p-6 rounded-2xl border border-zinc-900 mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <input type="text" placeholder="الاسم *" required value={name} onChange={(e) => setName(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]" />
        <input type="number" placeholder="السعر *" required value={price} onChange={(e) => setPrice(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]" />
        <input type="number" placeholder="الخصم %" value={discount} onChange={(e) => setDiscount(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]" />
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]">{categories.map(cat => <option key={cat._id} value={cat.name}>{cat.name}</option>)}</select>
        ...
        <select value={type} onChange={(e) => setType(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]"><option value="normal">عادي</option><option value="box">بوكس مخصص</option></select>
        {type === 'box' && <input type="number" placeholder="أقصى عدد *" value={maxItems} onChange={(e) => setMaxItems(e.target.value)} className="bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]" />}
        ...
        {type === 'box' && (
          <div className="md:col-span-2 bg-black p-4 rounded border border-zinc-800">
            ...
            <div className="flex gap-2 mb-2"><input type="text" value={boxItemName} onChange={e=>setBoxItemName(e.target.value)} className="flex-1 bg-[#110808] border border-zinc-800 rounded p-2 text-sm outline-none focus:border-[#e11d48]"/><button type="button" onClick={()=>handleListAdd(boxItemName, null, boxItemsList, setBoxItemsList, setBoxItemName, null)} className="bg-zinc-800 px-3 rounded">+</button></div>
        ...
        <div className="bg-black p-4 rounded border border-zinc-800">
          ...
          <div className="flex gap-2 mb-2"><input type="text" placeholder="الحجم" value={sizeName} onChange={e=>setSizeName(e.target.value)} className="w-1/2 bg-[#110808] border border-zinc-800 rounded p-2 text-xs outline-none focus:border-[#e11d48]"/><input type="number" placeholder="السعر" value={sizePrice} onChange={e=>setSizePrice(e.target.value)} className="w-1/4 bg-[#110808] border border-zinc-800 rounded p-2 text-xs outline-none focus:border-[#e11d48]"/><button type="button" onClick={()=>handleListAdd(sizeName, sizePrice, sizesList, setSizesList, setSizeName, setSizePrice)} className="bg-zinc-800 px-2 rounded">+</button></div>
        ...
        <div className="bg-black p-4 rounded border border-zinc-800">
          ...
          <div className="flex gap-2 mb-2"><input type="text" placeholder="الإضافة" value={addonName} onChange={e=>setAddonName(e.target.value)} className="w-1/2 bg-[#110808] border border-zinc-800 rounded p-2 text-xs outline-none focus:border-[#e11d48]"/><input type="number" placeholder="السعر" value={addonPrice} onChange={e=>setAddonPrice(e.target.value)} className="w-1/4 bg-[#110808] border border-zinc-800 rounded p-2 text-xs outline-none focus:border-[#e11d48]"/><button type="button" onClick={()=>handleListAdd(addonName, addonPrice, addonsList, setAddonsList, setAddonName, setAddonPrice)} className="bg-zinc-800 px-2 rounded">+</button></div>
        ...
        <textarea placeholder="وصف الصنف..." rows="2" value={description} onChange={(e) => setDescription(e.target.value)} className="md:col-span-2 bg-black border border-zinc-800 rounded p-3 text-sm outline-none focus:border-[#e11d48]" />
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat._id} className="bg-[#110808] border border-zinc-900 rounded-2xl p-6">
              <h4 className="font-anton text-xl text-[#e11d48] tracking-widest mb-4" dir="ltr">{cat.name}</h4>
              <div className="space-y-3">
                {catItems.map((item) => (
                  <div key={item._id} className="bg-black border border-zinc-800 p-3 rounded flex justify-between items-center">
                    <div className="flex gap-4 items-center" dir="ltr"><img src={item.image} className="w-12 h-12 rounded object-cover bg-zinc-900" /><div><p className="font-bold text-sm">{item.name}</p><p className="text-[#e11d48] font-anton">{item.price} EGP</p></div></div>
    ...
    <div dir={lang === 'ar' ? 'rtl' : 'ltr'} className="bg-[#050000] min-h-screen">
      ...
      {selectedItemDetail && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[100] p-4 font-cairo">
          <div className="bg-[#110808] border border-zinc-900 rounded-2xl max-w-md w-full p-6 relative shadow-2xl">
            ...
            <h3 className="text-2xl font-anton text-[#e11d48] tracking-widest mb-6" dir="ltr">{selectedItemDetail.name}</h3>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {selectedItemDetail.sizes.map((sz, idx) => (
                    <div key={idx} onClick={() => setSelectedSize(sz)} className={`p-3 rounded border cursor-pointer text-center transition ${selectedSize === sz ? 'bg-[#e11d48]/20 border-[#e11d48] text-[#e11d48]' : 'bg-black border-zinc-800 text-zinc-400'}`}>
                      <span className="block font-bold text-sm">{sz.name}</span><span className="block font-anton tracking-wider mt-1" dir="ltr">{getDiscountedPrice(sz.price, selectedItemDetail.discount)} EGP</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {selectedItemDetail.addons.map((addon, idx) => (
                    <label key={idx} className={`p-3 rounded border cursor-pointer flex items-center justify-between transition ${selectedAddon === addon ? 'bg-[#e11d48]/20 border-[#e11d48] text-[#e11d48]' : 'bg-black border-zinc-800 text-zinc-400'}`}>
                      <div className="flex items-center gap-3"><input type="radio" checked={selectedAddon === addon} onChange={() => setSelectedAddon(addon)} className="hidden" /><span className="font-bold text-sm">{addon.name}</span></div><span className="font-anton tracking-widest" dir="ltr">+{addon.price} EGP</span>
                    </label>
                  ))}
                </div>
              </div>
            )}
            <button onClick={handleAddCustomizedItemToCart} className="w-full bg-[#e11d48] text-white py-3.5 rounded font-bold" dir="ltr">إضافة للسلة • {currentItemTotalPrice} EGP</button>
          </div>
        </div>
      )}
      ...
      {isBoxModalOpen && activeBox && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[100] p-4 font-cairo">
          <div className="bg-[#110808] border border-zinc-900 rounded-2xl max-w-md w-full p-6 relative shadow-2xl">
            ...
            <h3 className="text-2xl font-anton text-[#e11d48] tracking-widest mb-2" dir="ltr">{activeBox.name}</h3>
